...
namespace YouTubePlays.Discord.Bot.Keyboard
{
    public class Generation5Keyboard : AbstractKeyboard
    {
        ...
        public override string ShortKey { get; } = "5";

        protected override KeyboardOptions KeyboardOptions { get; } = new KeyboardOptions
        {
            KeyMapSizes = new[] { (12, 5), (12, 5), (12, 5), (-1, -1) },
            PreExecuteCommand = "st,d",
            StartingPosition = (10, 0),
            ModeSwitchDelay = 4,
            PostModeSwitchCommand = "d",
            PostModeSwitchPosition = new[] { (0, 0), (2, 0), (3, 0), (-1, -1) }
        };

        protected override TouchOptions TouchOptions { get; } = new TouchOptions
        {
            TouchAvailable = true,
            PreExecuteCommand = "t:14:89,p2",
            ModeSwitchDelay = 2,
            ModeSwitchButton = new[] { (14, 89), (23, 89), (32, 89) }
        };
        ...
        protected override Dictionary<string, KeyMapping[]> CharMappings { get; } = new Dictionary<string, KeyMapping[]>
        {
            ...
            { "!", new[] { new KeyMapping(2, 4, 0, 37, 27) } },
            { "?", new[] { new KeyMapping(2, 5, 0, 44, 27) } },
            ...
            { "<:)>", new[] { new KeyMapping(2, 4, 4, 37, 77) } },
            { "<:D>", new[] { new KeyMapping(2, 5, 4, 44, 77) } },
...

            {
                " ", new[]
                {
                    ...
                    new KeyMapping(0, 0, 3, 12, 64), new KeyMapping(0, 1, 3, 19, 64), new KeyMapping(0, 2, 3, 25, 64), new KeyMapping(0, 3, 3, 31, 64), new KeyMapping(0, 4, 3, 37, 64), new KeyMapping(0, 5, 3, 44, 64), new KeyMapping(0, 6, 3, 50, 64), new KeyMapping(0, 7, 3, 56, 64), new KeyMapping(0, 8, 3, 62, 64), new KeyMapping(0, 9, 3, 68, 64), new KeyMapping(0, 10, 3, 75, 64), new KeyMapping(0, 11, 3, 81, 64), new KeyMapping(0, 12, 3, 87, 64),
                    ...
                    new KeyMapping(1, 0, 3, 12, 64), new KeyMapping(1, 1, 3, 19, 64), new KeyMapping(1, 2, 3, 25, 64), new KeyMapping(1, 3, 3, 31, 64), new KeyMapping(1, 4, 3, 37, 64), new KeyMapping(1, 5, 3, 44, 64), new KeyMapping(1, 6, 3, 50, 64), new KeyMapping(1, 7, 3, 56, 64), new KeyMapping(1, 8, 3, 62, 64), new KeyMapping(1, 9, 3, 68, 64), new KeyMapping(1, 10, 3, 75, 64), new KeyMapping(1, 11, 3, 81, 64), new KeyMapping(1, 12, 3, 87, 64),
